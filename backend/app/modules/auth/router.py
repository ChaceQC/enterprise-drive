...
from app.infrastructure.captcha.base import CaptchaVerifier
...
def get_auth_service(
    ...
    captcha_verifier: Annotated[CaptchaVerifier, Depends(get_captcha_verifier)],
) -> AuthService:
    return AuthService(
...
async def list_browser_sessions(
    ...
    return await service.list_browser_sessions(
        current_user=current_user,
        session_token=_require_session_cookie(http_request),
    )


@router.delete("/sessions/{session_id}", response_model=BrowserSessionRevokeResponse)
async def revoke_browser_session(
    ...
    result = await service.revoke_browser_session(
        current_user=current_user,
        current_session_token=_require_session_cookie(http_request),
