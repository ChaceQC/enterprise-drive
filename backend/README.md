...
预览 PDF 需要安装 Poppler，并确保 `pdftoppm` 在 `PATH` 中；Office 预览后续接入 LibreOffice headless 后也应按同样方式安装系统工具，不在业务代码中自研解析器。
...
- 预览基础链路：上传成功后写入 `preview.render_requested` outbox event，`preview.dispatch_outbox` 消费事件并使用 Pillow 生成图片 WebP 预览产物；PDF 会通过 Poppler `pdftoppm` 在临时目录中渲染第一页 PNG，再复用 Pillow 生成 WebP。产物写入私有对象存储 `previews/{tenant_id}/{node_id}/{version_id}/image.webp`；`GET /api/v1/files/{node_id}/preview` 会校验节点级 `preview` 权限并返回短期私有预览 URL。当前 Office 预览仍待接入 LibreOffice headless 等成熟开源工具。
