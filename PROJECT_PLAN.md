...
2026-08-04 已开始收敛 `backend-ci` 的重复执行：新增可测试的变更范围路由和同分支并发取消，backend、Rust、Windows 部署、安装包、MinIO 与依赖策略按受影响文件运行；后端静态门禁前置，桌面 Rust job 移除与 Tauri 安装包重复的 release build，Tauri CLI 改为固定版本的 npm 二进制并缓存，MinIO Server/Client 合并到一个供应链 runner。每周定时任务继续覆盖 MinIO 与 Rust 依赖数据库漂移，手工触发保留完整门禁；完成本地 workflow/actionlint 验证后，以一次真实全 scope 运行确认最终节省和兼容性。
