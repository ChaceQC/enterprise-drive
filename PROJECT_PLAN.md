...
- 权限缓存和失效事件。（已完成 `permission.changed` outbox 事件写入，后续补 Redis 缓存消费和批量权限评估）
- 高危操作二次查库。
