...
- 预览任务和搜索索引任务。（已完成搜索索引/抽取入口、图片 WebP 预览和 PDF 首页 WebP 预览基础链路；Office 预览待接 LibreOffice 等成熟开源工具）
- 权限过滤与二次校验。
