...
- 内部分享和外链分享。（已完成基础表、迁移、服务层、创建/详情/撤销 HTTP API 和带租户边界的外链访问校验入口；外链下载入口待接入）
- 提取码、过期、次数限制、撤销。（已完成提取码哈希、过期/次数字段、撤销服务、外链访问次数原子扣减、IP 总量和 `token + IP` 维度限流；下载次数并发扣减待接入）
- 预览任务和搜索索引任务。
- 权限过滤与二次校验。
