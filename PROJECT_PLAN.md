...
- 内部分享和外链分享。（已完成基础表、迁移、服务层和创建/详情/撤销 HTTP API；外链访问入口待接入）
- 提取码、过期、次数限制、撤销。（已完成提取码哈希、过期/次数字段和撤销服务；外链访问校验与并发扣减待接入）
- 预览任务和搜索索引任务。
- 权限过滤与二次校验。
