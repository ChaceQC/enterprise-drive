...
2026-07-31 `BE-030` 首轮安全测试修复已完成本地闭环：对当前虚拟环境 123 个实际包的审计发现 20 条记录全部来自攻击者可达的图片预览依赖 `Pillow 12.2.0`，已升级并锁定 `Pillow 12.3.0`；加入项目内 `bandit` 与 `pip-audit` dev 依赖和 CI 门禁，升级后审计 145 个环境包为 0 已知漏洞，Bandit 中危/高危为 0。登录新增来源 IP 与账号哈希双维 Redis 固定窗口，不存在租户/用户时仍执行 Argon2id dummy verify；422 校验错误不再回显原始 `input`。完整后端门禁为 `172 passed, 4 skipped`，runtime/preview 镜像均已受限重建并确认包含 `Pillow 12.3.0`；隔离真实 Compose 验证错误登录首次返回 `401`、同账号第二次返回 `429`，峰值为 5 个运行容器、123.1% aggregate Docker CPU 和 1426.3 MiB 容器内存，结束后容器、网络和卷均为 0。完整 route 权限矩阵、生产 Settings 自校验和动态安全测试仍需继续推进，不能把首轮依赖与认证修复当作全部安全验收。
