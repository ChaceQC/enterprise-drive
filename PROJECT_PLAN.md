...
- 搜索 ACL 事件和文件索引写入。（已完成 ACL token builder、`search.acl_rebuild_requested` outbox event、上传完成后的 `search.index_requested` 事件、OpenSearch 文件索引写入入口和 ACL 变更后的保守范围重建；查询过滤后续接入）
- 高危操作二次查库。
