...
- Windows 11 Docker Desktop 正式部署：多阶段 Dockerfile、根 `compose.windows.yml`、Compose 内 Nginx gateway、根 `.env.windows.example` 和 `deploy/windows/manage.ps1`。（已完成本机 HTTP 基线、公网 ACME/TLS 配置和续期命令；真实 DNS/受信证书验收待生产环境执行）
...
- named volumes、真实 `/readyz` PostgreSQL 探针、Preview Worker 资源限制、定时维护任务和数据库连接池边界。（已完成）备份、发布和回滚目前只有手工检查清单；PostgreSQL/MinIO/证书卷自动化备份恢复、同一业务时间点隔离演练、告警和治理看板仍待完成。
- Kubernetes、systemd 降为未来可选迁移方案，不作为当前交付目标。

## 5. 当前下一步

2026-07-15 已在 2026-07-14 Windows Docker 本机闭环基础上完成公网 TLS 配置闭环：根 `compose.windows.yml`、`.env.windows.example`、多阶段后端/Preview 镜像、Compose 内 Nginx gateway、`deploy/windows/manage.ps1`、真实数据库 `/readyz`、Celery beat、S3 内外端点和 Worker NullPool 边界均已实测；公网侧已补 ACME HTTP-01 bootstrap、Certbot 证书卷、TLS 1.2/1.3 server block、HTTP `308` 跳转、`80/443` Host 分流、续期/热重载编排和 Windows 计划任务管理。本机已用自签名双域名证书在标准宿主 `80/443` 启动并清理完整隔离编排；真实受信证书签发、外部 DNS/网络和实际 renewal lineage 续期仍需生产环境验收。下一步先补 PostgreSQL/MinIO 自动化备份恢复、固定 MinIO 镜像和供应链扫描，再继续以下业务治理任务。
