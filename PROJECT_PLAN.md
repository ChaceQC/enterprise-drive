...
2026-08-07 Sprint 13 `upload_init` 代码侧收口（提交 `a5e44af`）：在
...
`upload_init` P95 `470 ms` 超过 `300 ms`，不得伪装成通过。另修复了 pytest 收集
Locust/gevent late monkey-patch 的 `RecursionError`，提交 `a5e44af` 已推送；定向
重跑的 backend CI run `31135045950` 全绿。下一步是在最终候选发布窗口再执行一次
target/mixed 复验。
