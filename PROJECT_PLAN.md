...
- 搜索 ACL 事件。（已完成 ACL token builder、`search.acl_rebuild_requested` outbox event 和 `search.dispatch_outbox` 队列入口，OpenSearch 写入后续接入）
- 高危操作二次查库。
