...
- 数据库：PostgreSQL 16+。
- 缓存与限流：Redis。
- 对象存储：S3 兼容存储；正式和本地依赖 Compose 均使用固定 digest 的 SeaweedFS `4.40`，后端通过 `StorageAdapter` 与通用 S3 客户端隔离服务端实现。
- 搜索：OpenSearch。
- 异步任务：Celery。
...
- SQLAlchemy、Alembic、PostgreSQL、Redis、S3 兼容对象存储本地 compose。
...
- 企业级补强计划：对象存储稳定性、同 hash 并发、配额账户/策略管理、下载自动选路、图片/PDF 水印、关键字 DLP 和目标规模性能门禁均已闭环；multipart 控制面通过 MinIO Python SDK 的公共预签名 API 生成标准 S3 HTTP POST/DELETE，不绑定 MinIO 服务端，也不调用 SDK 私有方法。

### 上传下载企业级补强计划

上传/下载链路、秒传、multipart、服务端 hash 校验、容量账本和彻底删除容量释放是当前最重要的企业级主链路。2026-08-03 已按阶段表完成本节原有补强项；以下记录当前已落地结果及继续留在后续治理中的真实边界：

- multipart create/complete/abort 已改为 `S3MultipartControlClient`：通过 MinIO Python SDK 的公共 `get_presigned_url` 生成内部短期控制 URL，再发送标准 S3 HTTP POST/DELETE 和 XML 请求，不再依赖 `_create_multipart_upload`、`_complete_multipart_upload`、`_abort_multipart_upload`。客户端依赖范围锁定为 `minio>=7.2.20,<8`，控制请求超时与 URL 有效期可配置；升级必须通过真实 S3 兼容集成门禁。
- 对象复制到最终 `objects/{tenant_id}/{hash_prefix}/{content_hash}` 成功但数据库最终化失败后，可能出现孤儿最终对象。`file.cleanup_orphaned_objects` 反向扫描任务、`BE-031/BE-032` 的真实 S3 主路径及 `BE-035` 连续失败/stale 指标告警均已完成；Alertmanager 企业 webhook 路由和 Grafana overview/maintenance 看板已纳入可选 `monitoring` profile，实际 webhook 端点与值班流程由生产环境配置。
...
- `BE-029` 目标门禁已覆盖 10,000 节点、100 万 OpenSearch 文档和 1,000 万审计日志；当前 SeaweedFS 候选的 `upload_complete` 工件计入 1,934 个样本、0 失败，吞吐 `58.0579 RPS`，不含对象存储合并的 API P95 `600 ms`，端到端 P95 `650 ms`，并清理 2,000/2,000 个准备节点。最新有效 `mixed` 工件位于 `backend/tmp/performance/20260807-sprint13-a5e44af/mixed-target-workers8-final/report.json`，对应 `ca83d94`、43,403 个请求、0 失败，`admin_audit`/`file_list_permission_batch`/`search` 通过，`upload_init` P50 `260 ms`、P95 `540 ms`、P99 `790 ms`，未达到 `300 ms` 目标，报告必须保持 `passed=false`；旧 45,535 请求/470 ms 工件仅保留为历史证据。
...
- API、migration、seed、`storage-init`、按队列隔离的 Worker、Celery beat、PostgreSQL、Redis、SeaweedFS、OpenSearch 的完整编排；只有 gateway 发布宿主端口，默认 HTTP `18080/19000`，公网模式由 gateway 发布 `80/443` 并按 API/存储域名 Host 分流。（已完成）
...
- `QA-001`/`QA-002` 已接入：后端 CI 在同一次 pytest 中运行 PostgreSQL、Redis、S3 兼容对象存储、OpenSearch 四依赖健康、逐项故障探测与恢复矩阵并上传报告；OpenAPI 归档、TypeScript client、route matrix 和 breaking-change 门禁继续保持。既有 `BE-029` 目标规模性能证据复用，不重复执行无关性能 target。
...
2026-08-07 Sprint 13 继续收口（当前候选 `ca83d94`）：最新有效 mixed 工件
`backend/tmp/performance/20260807-sprint13-a5e44af/mixed-target-workers8-final/report.json`
记录 43,403 个请求、0 失败，`upload_init` P95 `540 ms`，仍未达到 `300 ms` 门禁。
在已有单次 blob 查询优化基础上，当前工作树新增 quota 空间/租户/用户账户一次批量读取，
缺失账户继续沿用并发安全的 `ensure_account()`；定向配额用例 `2 passed`，受影响
Ruff/format 和 mypy 通过，未运行性能负载。下一步为 `upload_init` 增加仅 benchmark
请求启用的分段计时，依据证据做最小修复后再执行一次最终 target/mixed；不重跑已通过
`upload_complete` 或无关集合。

2026-08-07 上一小步（提交 `a5e44af`）：在 `UploadService.init_upload()` 中将 active
blob 查询与 any-status 查询合并为一次读取，保留 active 秒传、非 active
`BLOB_DELETING` 和无记录 multipart 三条语义；直接受影响上传行为 `2 passed`，受影响
Ruff/format 和 `git diff --check` 通过。另修复了 pytest 收集 Locust/gevent late
monkey-patch 的 `RecursionError`，定向重跑的 backend CI run `31135045950` 全绿。
...
2026-07-31 已把此前仅停留在接口示例、技术建议或“后续接入”的能力补成 Sprint 9 至 Sprint 13、工程任务和远期 Backlog。2026-08-03 Sprint 6 和 Sprint 9 的代码侧能力已完成；当时规划的 `v0.4.0` 未进入正式发布，其生产 DNS/受信证书记录和 MinIO 修复镜像门禁在当时继续阻塞 `v1.0.0`。当时的并行顺序为：外部发布门禁不伪造完成，同时开始桌面端依赖的设备会话与增量同步契约；随后推进 Sprint 7/8 Rust 桌面端、Web 用户端与管理后台、身份安全、规模治理和 `v1.0.0` 稳定发布。Web 页面不得反向定义后端业务规则。MinIO 运行时替换的后续结果以上方 2026-08-05 最新记录为准。

2026-07-31 已把 Rust 桌面客户端从笼统的二期增强项提升为 Sprint 7 和 Sprint 8 正式路线。当时仓库仍没有桌面客户端代码；其后已按 `0.5.0` 目标完成 Sprint 7 的架构 ADR、后端设备会话/增量同步契约、Rust API client、本地索引和单向传输，下一阶段进入双向同步、冲突处理和签名发布；当时正式发布不得绕过 MinIO 风险门禁。当前对象存储发布门禁以上方 2026-08-05 SeaweedFS 记录为准。
...
剩余边界必须保持明确：Windows CMS 只加密 `.env.windows`，其余数据库、对象、索引、队列和 TLS 卷归档仍依赖 BitLocker、restricted NTFS ACL 与加密外部介质；SHA-256 只校验完整性，不认证制作者身份；Redis/OpenSearch 原始卷只支持相同 image reference/image ID、单节点同拓扑；`-ForceRestore` rollback 是尽力恢复，异常时受限 ACL 归档会保留并报告路径。2026-08-03 扫描复核把 MinIO Server/Client Critical 唯一 ID 允许集收紧为 16/9，其中两个 MinIO 自身 Critical 在固定社区镜像中没有 patched version；当时配置关闭 OIDC/LDAP/Console 只属于可达性缓解。当时规划的 `v0.4.0` 未发布，同一修复镜像、SBOM/Grype 重扫和真实 MinIO/备份恢复兼容性门禁继续阻塞 `v1.0.0`。该历史 blocker 的运行时替换结果以上方 2026-08-05 SeaweedFS 记录及 `docs/minio-security-risk.md` 为准。
