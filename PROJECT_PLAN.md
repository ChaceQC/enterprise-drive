...
2026-08-01 `BE-030` 已完成 route、租户、对抗输入与网络层安全矩阵：运行时 OpenAPI 36 个 route 全部登记并执行匿名请求，20 个副作用 route 完成登录态 CSRF 门禁，真实第二租户的 26 个内部资源 route 全部验证隐藏，活跃成员会话在撤权后立即失去访问；损坏/超大图片、文档路径与扩展名注入、Range 授权、预签名 URL 和轮换 token 外链穷举已纳入测试。Trusted Host 与 CORS 已完成 ASGI 动态测试；限资源真实 Nginx raw HTTP smoke 已验证 CL/TE 冲突、重复 Content-Length、API 413 和 storage `100 Continue`，并固化进 CI。下一步对账 `BE-031`、`BE-032` 的既有完成证据，再继续最早仍未完成的 `BE-029 target` 子任务。
