...
- 预览任务和搜索索引任务。（已完成搜索索引/抽取入口和图片 WebP 预览基础链路；Office/PDF 等复杂格式待接成熟开源工具）
- 权限过滤与二次校验。
