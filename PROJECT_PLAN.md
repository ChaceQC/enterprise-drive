...
2026-07-31 本机 Docker Desktop 已重新安装并恢复 `desktop-linux`：Docker client/server `29.6.2`、Linux `amd64` daemon 和 Docker Compose `v5.3.1` 可用；`compose.windows.yml` 使用 `.env.windows.example` 静态校验通过并解析出 15 个默认服务。已按 Compose 官方引用串行准备 Nginx、Certbot、PostgreSQL、Redis、MinIO Server/Client、OpenSearch 和 Python build base，当前没有运行中或残留容器；项目 runtime/preview 镜像仍需作为独立步骤构建。此前已真实启动 PostgreSQL、通过 `pg_isready`、从空库执行完整 Alembic 链到 `20260731_0013`，并运行回收站清理 PostgreSQL 集成测试；临时容器已删除。

2026-07-31 `BE-028` 在首次备份恢复测试中暴露资源失控：旧集成入口把项目镜像构建、15 服务启动、备份、成功 `backup-verify` 和恢复串成一个长任务，辅助 `docker run` 没有 CPU/内存/PID 边界，导致 CPU 长时间接近满载和 Docker Desktop 内存压力。修复计划已落地到代码：普通 `manage.ps1 up` 使用 `--no-build --pull never`；备份辅助容器默认限制 `0.50 CPU / 512m / 128 PIDs`、禁用额外 swap、gzip 与 `pg_dump` 压缩等级为 `1`；正式归档取消创建后的重复完整列表扫描但保留发布前安全校验；真实 integration 删除 `-BuildImages`，增加 `-PreflightOnly`、本地镜像快速门禁、`COMPOSE_PARALLEL_LIMIT=1`、Worker concurrency `1`、低资源覆盖、分阶段耗时和默认数据服务恢复验证。完整 target 全栈恢复改为显式 `-FullStackRestore`。当前只执行静态、烟雾和受限单容器验证，待项目镜像独立构建并通过 preflight 后，再在明确测试窗口运行完整 Compose 门禁。
