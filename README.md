...
当前已进入 Sprint 4 权限系统，已新增 `space_members` 基础表，创建空间时会自动写入当前用户的 `owner` 角色成员关系。空间、文件树、上传和下载 API 的实际访问边界仍暂时沿用“当前租户 + 空间拥有者”，后续将接入空间成员检查、目录 ACL、继承权限和拒绝优先策略。最终对象的 DB 驱动清理已由 `file.cleanup_unreferenced_blobs` 承担；对象存储中没有 DB 元数据的孤儿对象扫描仍作为后续治理任务处理。
