...
- Sprint 9 / `0.6.0`：文件版本核心 API 已完成，继续交付回收站列表、批量文件操作、内部分享接收端和完整管理 API。
...
`BE-030` 已完成 API 与网络安全矩阵；加入三个文件版本入口后运行时 OpenAPI 为 40 个 route，其中排除 ping/login 后的 38 个受保护或业务入口均继续纳入矩阵。损坏/超大图片、文档路径与扩展名注入、Range 权限、预签名 URL 和轮换 token 外链穷举也已覆盖。Trusted Host/CORS 已通过动态测试，真实 Nginx raw HTTP smoke 已验证 CL/TE 冲突、重复 Content-Length、API 请求体 413 与 storage 流式入口，并固化进 CI。
