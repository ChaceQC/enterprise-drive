...
Sprint 3 已落地上传会话基础：`upload_sessions`、`upload_parts` 迁移，S3/MinIO 对象存储适配器，`POST /api/v1/uploads/init` 初始化上传，`GET /api/v1/uploads/{session_id}` 查询状态，`POST /api/v1/uploads/{session_id}/parts/{part_no}/presign` 获取分片上传预签名 URL，`POST /api/v1/uploads/{session_id}/complete` 完成 multipart 上传并创建文件节点、blob 和首个版本，`POST /api/v1/uploads/{session_id}/abort` 取消未完成上传。初始化时若命中同租户同 hash、同大小的 `file_blobs`，会走秒传分支并创建文件节点和版本，同时增加 blob 引用计数并写入审计与 outbox。

当前的权限边界仍是临时实现：空间、文件树和上传 API 仅允许当前租户下的空间拥有者访问，目录 ACL、空间成员和角色将在 Sprint 4 权限系统中接入。下载预签名 URL、容量账本、服务端 hash 校验、最终对象 key 规整和上传清理任务仍在 Sprint 3 后续步骤中实现。
