...
当前的权限边界仍是临时实现：空间、文件树、上传和下载 API 仅允许当前租户下的空间拥有者访问，目录 ACL、空间成员和角色将在 Sprint 4 权限系统中接入。最终对象的 DB 驱动清理已由 `file.cleanup_unreferenced_blobs` 承担；对象存储中没有 DB 元数据的孤儿对象扫描仍作为后续治理任务处理。
