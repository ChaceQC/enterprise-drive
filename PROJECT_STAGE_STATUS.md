...
- 后端核心能力、权限、分享、预览、搜索、Windows 11 Docker 部署和基础治理已经形成试点基础，Sprint 3 已无剩余项。
...
- 最新数据库迁移：`20260803_0018`
- 当前缺少目录：`frontend/`、`desktop/`
- 当前未实现：内部分享接收端、除配额/文件安全策略外的完整管理 API、通用创建类幂等，以及大目录权限重算/治理页面
...
| Sprint 6：管理、治理、上线 | 管理员审计查询、配额账户/策略管理、文件安全策略管理、生命周期清理、指标与 Tracing、Windows 11 Docker Compose、Nginx、TLS/ACME、备份校验恢复、安全门禁 | 其余完整管理 API、Alertmanager/Grafana 看板、生产 DNS/受信证书实测、周期恢复演练、备份轮换；正式 `v0.4.0` 发布标记 |
...
| Sprint 9：核心产品闭环 | `BE-033` 多维配额及管理 API、`BE-034` 受控代理/Range/自动选路/水印/DLP、`BE-035` 维护告警、`BE-036` 文件版本、`BE-037` 回收站/批量操作已完成 | 当前进入 `BE-038`；内部分享接收端和其余完整管理 API 仍待完成 |
...
| Sprint 12：规模化治理与内容能力 | 已有文件树大目录删除/恢复/彻底删除后台任务、部分清理、指标、基础预览和搜索能力 | 大目录权限重算/治理页面、OCR、审计分区、Outbox DLQ、治理看板、故障注入和完整集成矩阵 |
...
- 路由安全矩阵与跨租户集中验证：67 passed；当前 OpenAPI 48 个路径、58 个操作。
...
3. 补齐其余正式发布门禁：全量回归、生产 TLS/DNS、MinIO 风险治理和 `v0.4.0` 发布标记；Sprint 3 目标规模性能证据直接复用已通过工件。
