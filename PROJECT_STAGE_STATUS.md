...
| Sprint 13.4：性能与稳定性 | 目标规模、长时间运行、并发、依赖故障、恢复和资源边界 | 代码侧继续收口；现有 mixed 候选仅 `upload_init` 未过 | 最新有效工件 `backend/tmp/performance/20260807-sprint13-a5e44af/mixed-target-workers8-final/report.json` 记录 `ca83d94`、43,403 请求、0 失败、10,000 节点/100 万索引/1,000 万审计证据，`upload_init` P95 `540 ms`（目标 `300 ms`）且 `passed=false`；上传初始化已合并 blob 查询，当前工作树另有三维 quota 账户批量读取补丁 | 先完成 quota 批量读取补丁和 benchmark-only 分段计时，再以最终候选按发布窗口执行一次 target/mixed；另需长期 soak、升级恢复耗时与 RPO/RTO 实测；不重复已通过负载 |
...
| Sprint 13 目标工件 | `upload_complete`：1,934 个样本、0 失败、API P95 `600 ms`、端到端 P95 `650 ms`、吞吐 `58.0579 RPS`，`passed=true`；最新 `mixed`：43,403 个请求、0 失败，`admin_audit`/`file_list_permission_batch`/`search` 通过，`upload_init` P50 `260 ms`、P95 `540 ms`、P99 `790 ms`、`passed=false`；旧 45,535 请求/470 ms 工件保留为历史证据 |
| `upload_init` 直接受影响验证 | active 秒传与 deleting blob 拒绝：`2 passed`；quota 三维账户批量读取与额度保留/释放定向用例：`2 passed`；未重跑完整 upload、mixed 或 upload-complete 负载 |
...
| 未重复范围 | 本轮未重跑历史性能 target、完整 mixed/upload-complete、全量 Playwright、全量 pytest 和无关桌面集合；仅复核最新 JSON 工件并运行上传/配额直接受影响的定向用例 |
...
4. 完成 `upload_init` benchmark-only 分段定位和最小修复，以最终候选执行一次 target/mixed
   复验，并完成真实 UAT、soak、安全、升级、恢复、回滚和 RPO/RTO 签字；外部门禁关闭后
   使用最终候选生成正式镜像 digest 与 Release 资产。
