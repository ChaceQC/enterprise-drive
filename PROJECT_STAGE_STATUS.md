...
当前项目处于 **后端 v0.4.0、Sprint 6/Sprint 9 代码闭环与正式发布门禁阶段**：
...
- 真实生产 DNS/受信证书证据和 MinIO 修复镜像不在当前本机环境内，正式 `v0.4.0` tag/Release 保持阻塞；Web 用户端、Rust 桌面端、身份治理、规模化治理和 `v1.0.0` 尚未完成。
...
- 运行时 OpenAPI：73 个路径、99 个操作
- 最新数据库迁移：`20260803_0021`
- 当前缺少目录：`frontend/`、`desktop/`
- 当前未实现：通用创建类幂等、桌面设备会话/增量同步契约，以及大目录权限重算/治理页面
...
- 运行时 OpenAPI 与安全矩阵已同步为 73 个路径、99 个操作；写操作 CSRF 和管理员/登录态读取门禁的既有定向集合已通过。
...
1. 在真实生产网络执行 `tls-validate-public`，保存双域名 DNS、HTTP `308`、HTTPS readiness 和受信证书记录。
2. 采用受支持修复镜像或可审计补丁镜像解决 MinIO Critical，重新生成 SBOM/Grype 并运行真实 MinIO/备份恢复兼容门禁。
3. 上述发布阻塞解除后创建 `v0.4.0` tag/Release；Sprint 3 目标规模性能证据继续复用已通过工件。
