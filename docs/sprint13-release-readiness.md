...
| `REL-002` | 性能与安全 | 自动门禁已补强，长稳与最终扫描待执行 | 性能报告、soak 报告、SBOM、漏洞报告、MinIO 修复证据、非阻断风险接受 |
| `REL-003` | 安装、升级与回滚 | 自动路径已补强，待 `0.9.0 -> 1.0.0` 演练 | 升级时间线、备份/恢复、桌面自动回退证据 |
...
- 长期 soak 必须单独记录持续时间、稳态负载、资源曲线、错误率、任务积压、
  数据库连接池、Redis、OpenSearch、MinIO 与 Worker 恢复情况；既有 300 秒 target
  结果不替代 soak。
...
- 后端、Preview、Web、MinIO Server/Client 和 Windows 安装包均有对应 SBOM 或
  可审计组件清单。
- Grype 最终报告不得出现未评审的新 Critical；既有 MinIO Critical 必须按
  `docs/minio-security-risk.md` 采用受支持修复发行版或可审计补丁镜像关闭。
  这是正式 tag/Release 的不可豁免门禁，不能仅以风险接受替代；其他允许保留的发现
  才可记录具体 ID、可达性、补偿控制、责任人和到期日后进入正式风险接受。
...
5. 注入应用、PostgreSQL、Redis、MinIO 和 OpenSearch 失败，记录探测与恢复时间线。
...
| 镜像与供应链 | 待生产指定 | 安全负责人 | 修复镜像/补丁证据、digest、SBOM、扫描、非阻断风险接受 |
...
接受记录，但 MinIO 修复镜像、真实 DNS/TLS、企业 OIDC/LDAPS、告警接收、升级回滚、
恢复与 RPO/RTO 签字不得由风险接受替代：
