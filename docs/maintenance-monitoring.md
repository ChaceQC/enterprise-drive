...
maintenance Worker 在 Celery `task_postrun` signal 中记录最终状态，不修改九个任务的业务函数。状态写入 Redis：
...
- `grafana`：通过 gateway `/grafana/` 子路径访问，预置运行概览和维护治理两张看板。
...
- `GET /api/v1/admin/maintenance/tasks`：查询九个任务的连续失败、alert、stale 和时间戳。
